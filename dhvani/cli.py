"""Command-line interface for dhvani."""
...
def cmd_devanagari(args):
    ...
    import dhvani
    text = " ".join(args)
    print(dhvani.to_devanagari(text))
...
def cmd_same(args):
    ...
    same = dhvani.are_same(word1, word2)
...
def cmd_langs(args):
    ...
    import dhvani
    text = " ".join(args)
    result = dhvani.identify_languages(text)
...
def cmd_normalize(args):
    ...
    import dhvani
    text = " ".join(args)
    print(dhvani.normalize(text))


def cmd_stats():
    from dhvani.lexicon.lookup import get_lexicon_stats
    from dhvani.cache import get_cache_stats
