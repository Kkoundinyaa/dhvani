...
from dhwani.lexicon.lookup import lookup_ipa
...
def romanized_hindi_to_ipa(word: str, use_model: bool = True) -> str:
    ...
    from dhwani.cache import cache_get_ipa
    ...
    if use_model:
        try:
            from dhwani.ipa_pipeline import romanized_to_ipa_model, romanized_to_devanagari_model, is_available
            if is_available():
...
def devanagari_to_ipa(word: str) -> str:
    """Convert Devanagari Hindi text to IPA using espeak-ng.

    Args:
        word: Text in Devanagari script
    ...
    try:
        ...
        backend = EspeakBackend("hi", preserve_punctuation=False, with_stress=False)
        result = backend.phonemize([word])[0].strip()
        return result
    except Exception:
        # Fallback: basic Devanagari to IPA mapping
